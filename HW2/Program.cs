...
namespace HW2
{
	internal class Program
	{
		static void Main(string[] args)
		{
            ...
            Console.Write("The number of person: ");int n = int.Parse(Console.ReadLine());//the number of variables of arr 
			Person[] per = new Person[n];
			for(int i = 0; i < n; i++)
			{
                Console.WriteLine($"Enter information for person {i+1}:");
                per[i] = Person.inputPersonInfor();//enter input for var in arr
			}
			Person per1 = new Person("alex","St 19",8);
			Person.displayPerson(per1);

			per= Person.SortedBySala(per);//sort then show 
